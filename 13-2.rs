...
fn main() {
    ...
    let mut items = vec![
        List::Children(vec![List::Value(2)]),
        List::Children(vec![List::Value(6)])
    ];
    ...
    items.sort_unstable();
    let mut index_product = 1;
    for (index, item) in items.iter().enumerate() {
        if *item == List::Children(vec![List::Value(2)]) || *item == List::Children(vec![List::Value(6)]) {
