    ...
    fn parse() -> Self {
        ...
        while lines.peek().is_some() {
            ...
            let operation = match op_parts.next().unwrap() {
                "+" => {
                    match op_parts.next().unwrap().parse::<u64>() {
            ...
            /* Parse divisor */
            let line = lines.next().unwrap().unwrap();
            let divisor = line[21..].to_string().parse::<u64>().unwrap();

            /* If we let the simulation run for a while, and we never reduce
             * our worry level, we'll likely overflow regardless how many bits
             * we use for integers. Instead, we can recognize that all the divisors
             * are prime and are thus coprime, so we can multiple them together
             * and then every worry manipulation we can mod against that number
             * to keep the divisor check correct while keeping the worry number
             * small.
             */
            worry_mod *= divisor;
        ...
        Sim { monkeys, inspected_counts, worry_mod }
    }

    fn simulate(&mut self) -> u64 {
        let worry_mod = self.worry_mod;
        for round in 0..10000 {
            ...
            for i in 0..self.monkeys.len() {
                for item in self.monkeys[i].item_queue.drain(0..).map(|i| i % worry_mod).collect::<Vec<u64>>() {
                    ...
                    self.monkeys[dest].item_queue.push_back(result % self.worry_mod);
                }
            }
        }
